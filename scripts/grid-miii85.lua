...
local function tick()
	...
	if last > 0 then
		midi_note_off(notes[last], MIDI_VEL, midi_ch)
	end
	...
	elseif step_gate_mode >= 3 and step_gate_mode <= 6 then
		...
		if (step_count - 1) % (step_gate_mode - 2) ~= 0 then
			next_note = 0
		end
	...
	if next_note > 0 then
		midi_note_on(notes[next_note], MIDI_VEL, midi_ch)
	end
	last = next_note
...
function event_grid(x, y, z)
	...
	elseif y == 2 then
		...
		if pages[cur_page] ~= nil then
			pages[cur_page]:event_grid(x, y, z)
		end
	end
